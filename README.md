...
- 🤖 **AI chat box** — streams answers from the Anthropic API (Claude Opus), grounded in a system prompt built from `data/profile.ts`. Server-side key, rate-limited, token-capped.
...
### Anthropic API key (powers the chat box)
...
3. Add the env var **`ANTHROPIC_API_KEY`** under
