...
export function ProjectCard({ project, index }: { project: Project; index: number }) {
  return (
    <motion.article
      ...
      className="panel group flex flex-col overflow-hidden rounded-3xl transition-all duration-300 hover:-translate-y-1 hover:border-[var(--border-strong)] hover:shadow-2xl hover:shadow-accent-cobalt/10"
    >
      ...
      >
        ...
        <span className="absolute left-5 top-4 font-display text-5xl font-extrabold text-white/30 transition-all duration-500 group-hover:scale-110 group-hover:text-white/50 origin-bottom-left">
          {project.number}
        </span>
          ...
          <dl className="mt-4 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-xs">
            {project.specs.map((s) => (
              <div key={s.label} className="contents">
                <dt className="font-mono uppercase tracking-wider text-[var(--accent-text)]">
                  {s.label}
                </dt>
          ...
          <div className="mt-5 flex gap-3 text-sm">
            {project.repo && (
              <a
                ...
                className="inline-flex items-center gap-1.5 font-medium text-[var(--text)] transition-colors hover:text-[var(--accent-text)]"
              >
